...
void CustomPlugin::setToolbox(QGCToolbox* toolbox)
{
    ...
    connect(qgcApp()->toolbox()->corePlugin(), &QGCCorePlugin::showAdvancedUIChanged, 
...
{
    return QStringLiteral("/custom/img/custom_splash.png");
}
...
{
    // DO NOT call parent method - create completely independent engine
    QQmlApplicationEngine* qmlEngine = new QQmlApplicationEngine(parent);
    qmlEngine->addImportPath("qrc:/Custom/Widgets");
    
    // Create and register LoginHandler FIRST
    LoginHandler* loginHandler = new LoginHandler(qmlEngine);
    
    // Register UserSession as singleton BEFORE any QML loading
    qmlRegisterSingletonType<UserSession>("UserSession", 1, 0, "UserSession",
    ...
    qmlEngine->rootContext()->setContextProperty("loginHandler", loginHandler);
    qmlEngine->rootContext()->setContextProperty("UserSession", &UserSession::instance());
    
    // Debug resource availability
    QFile qmlFile(":/custom/qml/LoginView.qml");
    QFile usersFile(":/custom/qml/users.json");
    qDebug() << "LoginView.qml exists:" << qmlFile.exists();
    qDebug() << "users.json exists:" << usersFile.exists();
    
    // Load ONLY our login system
    qmlEngine->load(QUrl(QStringLiteral("qrc:/custom/qml/LoginView.qml")));
    
    if (qmlEngine->rootObjects().isEmpty()) {
        qDebug() << "CRITICAL: Failed to load LoginView.qml";
    } else {
        qDebug() << "SUCCESS: LoginView.qml loaded with" << qmlEngine->rootObjects().size() << "root objects";
    }
